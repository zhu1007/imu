...
CAN_BIT_MODIFY = 0x05
CAN_RX_STATUS = 0xB0
CAN_RD_RX0_BUFF = 0x90
CAN_RD_RX1_BUFF = 0x94
CAN_LOAD_TX = 0x40
...
class MCP2515:
    def __init__(self):
        self.spi = SPI(
            0, 10000_000, polarity=0, phase=0, sck=Pin(6), mosi=Pin(7), miso=Pin(4)
        )
    ...
    def bit_modify(self, addr, mask, data):
        """
        MCP2515 BIT MODIFY 指令
        addr: 寄存器地址
        mask: 位掩码，1表示要修改的位
        data: 新值
        """
        self.cs.value(0)
        self.spi.write(bytes([0x05, addr, mask, data]))
        self.cs.value(1)
    ...
    def Init(self, speed="125KBPS"):
        ...
        # Set RX
        #设置RXB0
        self.WriteBytes(RXB0SIDH, 0x00) # 清除过滤器
        self.WriteBytes(RXB0SIDL, 0x60) # 标准帧
        self.WriteBytes(RXB0CTRL, 0x60) # 接收所有报文
        ...
        #设置RXB1
        self.WriteBytes(RXB1SIDH, 0x00) # 清除过滤器
        self.WriteBytes(RXB1SIDL, 0x60) # 标准帧
        self.WriteBytes(RXB1CTRL, 0x60) # 接收所有报文
        self.WriteBytes(RXB1DLC, DLC_8)

        self.WriteBytes(RXF1SIDH, 0xFF)
        self.WriteBytes(RXF1SIDL, 0xE0)
        self.WriteBytes(RXM1SIDH, 0xFF)
        self.WriteBytes(RXM1SIDL, 0xE0)
    ...
    def read_rx0_buffer(self):
        ...
        # 批量读取所有需要的寄存器（从RXB0SIDH到RXB0D7，共13字节）
        # 读取命令：0x80 | 寄存器地址（读操作标志）
        self.cs.value(0)  # 使能SPI通信
        self.spi.write(bytearray([CAN_RD_RX0_BUFF]))  # 直接读取缓冲区
        buf = self.spi.read(13)  # 一次性读取13字节（ID4字节 + DLC1字节 + 数据8字节）
        self.cs.value(1)  # 禁用SPI通信

        # 解析ID部分（前4字节：SIDH, SIDL, EIDH, EIDL）
        sidh, sidl, eidh, eidl = buf[0], buf[1], buf[2], buf[3]

        # 解析数据长度（第5字节）
        dlc = buf[4] & 0x0F  # 只取低4位（DLC有效位）

        # 解析数据部分（接下来的8字节）
        data = list(buf[5 : 5 + dlc])
        ...
        return {"id": can_id, "data": data, "is_extended": is_extended}
    ...
    def read_rx1_buffer(self):
        ...
        """读取RXB0接收缓冲区数据，返回包含ID和数据的字典"""
        # 批量读取所有需要的寄存器（从RXB1SIDH到RXB1D7，共13字节）
        # 读取命令：0x80 | 寄存器地址（读操作标志）
        self.cs.value(0)  # 使能SPI通信
        self.spi.write(bytearray([CAN_RD_RX1_BUFF]))  # 发送读取命令
        buf = self.spi.read(13)  # 一次性读取13字节（ID4字节 + DLC1字节 + 数据8字节）
        self.cs.value(1)  # 禁用SPI通信
    ...
    def check_and_clear_interrupt(self):
        ...
        #         if intf & RX0IF_SET:
        #             # RXB0缓冲区有数据
        #             data1 = self.read_rx0_buffer()
        #             # 清除RX0中断标志
        #             self.WriteBytes(CANINTF, intf & ~RX0IF_SET)
        #         if intf & RX1IF_SET:
        #             # RXB1缓冲区有数据
        #             data2 = self.read_rx1_buffer()
        #             # 清除RX1中断标志
